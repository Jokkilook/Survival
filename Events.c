...
void run_tiger(Player* player) {
    if (player->SP < 30) {
        decreaseHP(player, 10);
        print_result("조금 긁혔지만 도망쳤다..");
    }
    else {
        decreaseSP(player, 30);
        print_result("멈추면 죽는다!!!!!!");
    }
}
//호랑이 유인
...
void ride_bicycle(Player* player) {
    if (player->SP > 30) {
        increaseSP(player, 30);
        print_result("탈출은 못해도 기분이 좋았다!");
    }
    else {
        decreaseHP(player, 50);
        print_result("크게 굴러떨어져버렸다...");
    }
}
...
void spin_bicycle(Player* player) {
    ...
    if (player->SP < 50) {
        decreaseHP(player, 50);
        print_result("산을 굴러떨어지고나서야 후회했다....");
    }
    else {
        increaseHP(player, 10);
        increaseSP(player, 50);
        print_result("완벽한 착지!");
    }
}
...
void give_gup_wolf(Player* player) {
    if (player->SP < 40) {
        decreaseHP(player, 30);
        print_result("소리 질렀다가 늑대가 물어뜯었다.....");
    }
    else {
        print_result("반복되고 강조되는 위협으로 늑대를 두렵게 했다!");
    }
}
...
void pet_wolf(Player* player) {
    if (player->SP < 10) {
        decreaseHP(player, 50);
        print_result("꼬리를 만지다 크게 씹혔다....");
    }
    else {
        increaseHP(player, 40);
        increaseSP(player, 20);
        print_result("늑대의 감촉이 치유되는 느낌...");
    }
}

// 늑대에게서 도망치기
void runaway_wolf(Player* player) {
    print_result("늑대가 멀뚱히 쳐다보며 그냥 보내준다.");
}
...
void chase_gunsound(Player* player) {
    if (player->SP < 60) {
        decreaseHP(player, 80);
        print_result("멧돼지인 줄 알고 총에 맞고 말았어....");
    }
    else {
        decreaseSP(player, 10);
        print_result("사냥꾼에게 위험하다고 훈계만 들었다...");
    }
}
...
void go_otherside(Player* player) {
    if (player->SP < 20) {
        decreaseHP(player, 30);
        print_result("기습적으로 멧돼지에게 들이받혔다!");
    }
    else {
        decreaseSP(player, 10);
        print_result("멧돼지가 튀어나왔지만 빠르게 피했다!");
    }
}
...
void sleep_floor(Player* player) {
    if (player->SP > 30) {
        print_result("깜빡 졸고 말았다");
    }
    else {
        decreaseHP(player, 20);
        print_result("깜빡 졸고 말았다. .....너무 춥다...");
    }
}
...
void broke_trap(Player* player) {
    ...
    if (player->SP < 50) {
        decreaseHP(player, 30);
        print_result("덫에 크게 다쳤다!");
    }
    else {
        decreaseSP(player, 20);
        print_result("빠른 순발력으로 덫을 해체했다");
    }
}
...
void eat_bait(Player* player) {
    ...
    if (player->SP < 30) {
        decreaseHP(player, 10);
        print_result("아얏! 빼려다 다칠 뻔했다!");
    }
    else {
        decreaseHP(player, 20);
        increaseSP(player, 40);
        print_result("맛있었지만 속이 좀 안 좋다....");
    }
}

// 덫 주변에 숨기
void hide_trap_side(Player* player) {
    increaseHP(player, 30);
    increaseSP(player, 20);
    print_result("덫에 꿩이 걸렸다! 꿩고기 마시쪙!");
}
...
void search_tent(Player* player) {
    decreaseSP(player, 10);
    if (player->SP < 30) {
        decreaseSP(player, 70);
        print_result("으악 해골!!! 못 볼 걸 봤다");
    }
    else {
        increaseHP(player, 30);
        decreaseSP(player, 20);
        print_result("스팸을 찾았다. 구린내가 좀 나지만...");
    }
}

// 텐트를 불태우기
void blaze_tent(Player* player) {
    decreaseHP(player, 60);
    print_result("엄청난 섬광과 폭음과 함께 텐트는 대폭발했다.");
}

// 텐트 무시
void ignore_tent(Player* player) {
    print_result("왠지 불길하다. 썩은내도 나고.");
}